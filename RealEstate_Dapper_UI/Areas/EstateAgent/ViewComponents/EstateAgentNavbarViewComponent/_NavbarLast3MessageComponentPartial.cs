...
namespace RealEstate_Dapper_UI.Areas.EstateAgent.ViewComponents.EstateAgentNavbarViewComponent
{
    public class _NavbarLast3MessageComponentPartial : ViewComponent
    {
        ...
        public async Task<IViewComponentResult> InvokeAsync()
        {
			...
			var responseMessage = await client.GetAsync("https://localhost:44326/api/Messages?id=" + id);
			if (responseMessage.IsSuccessStatusCode)
			{
				var jsonData = await responseMessage.Content.ReadAsStringAsync();
				var valeus = JsonConvert.DeserializeObject<List<ResultInBoxMessageDto>>(jsonData);
				return View(valeus);
			}
			return View();
		}
