...
namespace RealEstate_Dapper_UI.ViewComponents.Dashboard
{
	public class _DashboardLast4ContactListComponentPartial : ViewComponent
	{
		...
		public async Task<IViewComponentResult> InvokeAsync()
		{
			...
			var responseMessage = await client.GetAsync("https://localhost:44326/api/Contacts/GetLast4Contact");
			if (responseMessage.IsSuccessStatusCode)
			{
				var jsonData = await responseMessage.Content.ReadAsStringAsync();
				var valeus = JsonConvert.DeserializeObject<List<Last4ContactResultDto>>(jsonData);
				return View(valeus);
			}
			return View();
		}
